...
class AssistantUI:
    def __init__(self, root: tk.Tk, on_submit, title: str = "TORQUE", on_force_stop=None):
        ...
        # Safe style acquisition (works with plain Tk root too)
        try:
        ...
        self._title_lbl = tb.Label(title_row, text=f"◉ {title}",
                                   font=("Segoe UI Black", 20), foreground=JARVIS_PRIMARY)
        ...
        self.theme_btn = tb.Checkbutton(
            right, text="Dark", bootstyle="primary-round-toggle",
        ...
        self.mic = tb.Progressbar(mic_box, orient="horizontal", mode="determinate", length=200, maximum=100, bootstyle="info-striped")
        ...
        self.log = tb.ScrolledText(
            log_frame, wrap="word", state="disabled",
            font=("Consolas", 11),
            bg="#0e131a", fg="#e6e6e6", insertbackground="#ffffff",
            relief="flat"
        )
    ...
    def set_status(self, text: str): self.status_var.set(str(text or ""))
    def set_listening(self, is_listening: bool): self._is_listening = bool(is_listening)
    def set_speaking(self, is_speaking: bool): self._is_speaking = bool(is_speaking)
    def set_caption(self, text: str): self.caption_var.set(str(text or ""))

    def update_mic_level(self, level: int):
        try: self.mic["value"] = max(0, min(100, int(level)))
        except Exception: self.mic["value"] = 0

    def show(self, greeting_text: str = ""):
        try: self.root.deiconify(); self.root.lift(); self.root.focus_force()
        except Exception: pass
        if greeting_text: self.append(greeting_text, is_torque=True)
    ...
    def _toggle_theme(self):
        ...
        if mode == "light":
            ...
            try: self.root.configure(bg=BG_LIGHT)
            except Exception: pass
            ...
            self.style.theme_use("cyborg")
            self.current_theme = "dark"
            try: self.root.configure(bg=BG_DARK)
            except Exception: pass
